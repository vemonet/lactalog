...
import { fetchAll, type FeedingEntry, type MilkingEntry, type SleepEntry } from './sheets';

const [feeding, setFeeding] = createSignal<FeedingEntry[]>([]);
const [milking, setMilking] = createSignal<MilkingEntry[]>([]);
const [sleeping, setSleeping] = createSignal<SleepEntry[]>([]);
const [loading, setLoading] = createSignal(false);
const [error, setError] = createSignal('');
const [loadedOnce, setLoadedOnce] = createSignal(false);

export { feeding, milking, sleeping, loading, error, loadedOnce };

export async function refresh(): Promise<void> {
  setLoading(true);
  setError('');
  try {
    const { feeding: f, milking: m, sleeping: s } = await fetchAll();
